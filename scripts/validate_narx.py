"""Compare NARX A2C learning on one shared, audited B747 environment.
...
def main():
    ...
    actor, critic = Actor(6, 1), Critic(6)
    agent = A2CLearner(
        actor,
        critic,
        gamma=0.99,
    ...
    runner = Runner(ScalarEnv(), actor, agent.writer)
    ...
    def monitor(tag, scalar_value, *pos, **kw):
        ...
        if tag.startswith("loss/"):
            losses.append({"step": runner.steps, "tag": tag, "value": numeric})
    ...
    try:
        for update in range(args.updates):
            memory = runner.run(128)
            agent.learn(memory, runner.steps, discount_rewards=True)
            if not all(
        ...
        result["environment_steps"] = runner.steps
