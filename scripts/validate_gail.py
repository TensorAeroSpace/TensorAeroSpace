"""Compare GAIL learning on one shared, audited B747 environment.
...
def main():
    ...
    expert_env = ScalarEnv()
    data = []
    for _ in range(40):
        obs, _ = expert_env.reset()
        for _ in range(199):
            command = np.clip(-2.0 * obs[0] + 0.6 * obs[1], -1, 1)
            action = np.array([command], dtype=np.float32)
            data.append(np.concatenate([obs, action]))
            obs, _, term, trunc, _ = expert_env.step(action)
            if term or trunc:
                break
    demonstrations = np.asarray(data, dtype=np.float32)
    agent = GAIL(
        ScalarEnv(),
        learning_rate=3e-4,
        max_steps=128,
    ...
    networks = [agent.model, agent.discriminator]

    def policy(obs):
        return agent.model(obs)[0].mean.clamp(-1, 1)
    ...
    result = {
        ...
        "config": {
            ...
            "learning_rate": 3e-4,
            "expert_episodes": 40,
            "expert_transitions": len(demonstrations),
            "mini_batch_size": 64,
            "demonstrations_sha256": hashlib.sha256(
                demonstrations.tobytes()
            ).hexdigest(),
        },
    ...
    def monitor(tag, scalar_value, *pos, **kw):
        ...
        if tag.startswith("loss/"):
            losses.append({"step": agent.global_env_step, "tag": tag, "value": numeric})
        return original_scalar(tag, scalar_value, *pos, **kw)

    agent.writer.add_scalar = monitor
    started = time.monotonic()
    try:
        agent.learn(max_frames=args.frames, max_reward=float("inf"))
        ...
        result["policy_updates"] = agent.update_count
        result["discriminator_updates"] = agent.discrim_update_count
        torch.save(
            {
