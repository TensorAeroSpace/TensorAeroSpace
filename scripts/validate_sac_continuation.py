"""Compare SAC training in one call and in episode chunks on fixed B747 physics.

Use the same script and environment source for both revisions. The episode
budget and RNG stream are identical; evaluation restores training RNG states.
Target-update intervals above one expose resets at each train() boundary.
"""
...
def main():
    ...
    parser.add_argument("--target-interval", type=int, default=7)
    ...
    def checked_update(*pos, **kw):
        ...
        if not np.isfinite(values).all():
            raise FloatingPointError("Nonfinite SAC loss or entropy")
        max_abs_losses[:] = np.maximum(max_abs_losses, np.abs(values))
        updates_count += 1
        return tuple(values)

    agent.update_parameters = checked_update
    episode_count = 0
    log_episode = agent.writer.log_episode

    def monitor_episode(*pos, **kw):
    ...
    agent.writer.log_episode = monitor_episode
    started = time.monotonic()
    try:
        for _ in range(args.episodes if args.chunked else 1):
            agent.train(
                num_episodes=1 if args.chunked else args.episodes, verbose=False
            )
        if not all(torch.isfinite(p).all() for n in networks for p in n.parameters()):
            raise FloatingPointError("Nonfinite weights")
        result["gradient_updates"] = updates_count
        result["max_abs_losses"] = max_abs_losses.tolist()
        result["weight_sha256"] = {
            name: hashlib.sha256(
                b"".join(
                    value.detach().cpu().numpy().tobytes()
                    for value in getattr(agent, name).state_dict().values()
                )
            ).hexdigest()
