"""Generate data-driven illustration plots for the damage modeling docs.

Produces three PNG figures of *real numerical data* used in
``docs/{en,ru}/model/img/``:

- ``damage_strip_theory.png``     — ΔCy and ΔMx vs α
- ``damage_recompute_curves.png`` — m / S / Jx / cg_y vs loss fraction
- ``damage_demo_trajectory.png``  — healthy vs damaged 6-DoF trajectory

Two other illustrations under ``docs/{en,ru}/model/img/`` are
hand-crafted conceptual diagrams, NOT generated here — do not regenerate
them from this script:

- ``damage_section_layout.png``     — F-16 section layout (top-down)
- ``damage_event_lifecycle.png``    — end-to-end damage event lifecycle
- ``damage_symmetric_vs_asymmetric.png`` — sym vs asym wing-tip loss
- ``damage_event_timeline.png``     — DamageProfile timeline example
...
# ============================================================
def main():
    print("Generating data-driven damage docs plots...")
    print("(section_layout, event_lifecycle, symmetric_vs_asymmetric, "
          "and event_timeline are hand-crafted illustrations and are NOT "
          "regenerated by this script.)")

    print("\n[1/3] Strip-theory aero corrections")
    fig = plot_strip_theory()
    save_dual(fig, "damage_strip_theory.png", target="model")

    print("\n[2/3] Parameter recompute curves")
    fig = plot_recompute_curves()
    save_dual(fig, "damage_recompute_curves.png", target="model")

    print("\n[3/3] Demo trajectory: healthy vs damaged")
    fig = plot_demo_trajectory()
    save_dual(fig, "damage_demo_trajectory.png", target="model")

    print("\nAll data-driven plots saved.")
