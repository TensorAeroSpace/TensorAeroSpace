"""Train/evaluate SAC on F4CPitchEnvNormalized; retain finite-loss and tracking metrics.

Uses native observations/reward (reward multiplied by 100 for SAC) and fixes the
reference during each two-second episode. The same independent angle/rate envelope is enforced for both revisions.
"""
...
def main():
    ...
    class Tracking(gym.Env):
        ...
        def reset(self, *, case=None, **kwargs):
            ...
            self.env = F4CPitchEnvNormalized(
                initial,
        ...
        def step(self, action):
            ...
            failed = bool(abs(x[3]) > np.deg2rad(10) or abs(x[2]) > np.deg2rad(10))
            info["failed"] = failed
    ...
    def pd(obs):
        ...
        error_deg, q_deg_s = float(obs[0] * 30), float(obs[1] * 10)
        return np.array([np.clip((-0.7 * error_deg + 0.25 * q_deg_s) / 20, -1, 1)])
