"""Compare A2C and A2CWithNARXCritic learning on one shared, audited B747 environment.
...
def main():
    ...
    parser.add_argument("--critic", choices=["plain", "narx"], default="plain")
    ...
    from tensoraerospace.agent.a2c.model import A2C, A2CWithNARXCritic, Actor, Critic
    ...
    critic = (
        Critic(6)
        if args.critic == "plain"
        else NARXCritic(6, 1, 2, hidden_sizes=(64, 64))
    )
    cls = A2C if args.critic == "plain" else A2CWithNARXCritic
    agent = cls(
        ScalarEnv(),
        actor,
        critic,
        **({"history_length": 2} if args.critic == "narx" else {}),
        gamma=0.99,
        entropy_beta=0.001,
        actor_lr=4e-4,
        critic_lr=1e-3,
    ...
    def policy(obs):
        return actor(obs).mean.clamp(-1, 1)
    ...
    result = {
        ...
        "config": {
            ...
            "critic": args.critic,
            "history_length": 2 if args.critic == "narx" else 0,
            "rollout_steps": 128,
            "gamma": 0.99,
    ...
    def monitor(tag, scalar_value, *pos, **kw):
        ...
        if tag.startswith("loss/"):
            losses.append({"step": agent.steps, "tag": tag, "value": numeric})
    ...
    try:
        for update in range(args.updates):
            memory = agent.run_episode(128)
            agent.learn(memory, agent.steps, discount_rewards=True)
            if not all(
        ...
        result["environment_steps"] = agent.steps
