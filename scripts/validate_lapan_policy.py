"""Train/evaluate SAC on ImprovedLAPANEnv; retain finite-loss and tracking metrics.
...
def main():
    ...
    class Tracking(gym.Env):
        ...
        def step(self, action):
            ...
            failed = bool(abs(x[3]) > np.deg2rad(10) or abs(x[2]) > np.deg2rad(60))
            info["failed"] = failed
    ...
    def pd(obs):
        ...
        return np.array([np.clip((-0.15 * error_deg + 0.04 * q_deg_s) / 25, -1, 1)])
