...
def convert_tp_to_sec_tp(tp:np.array, dt:float=0.01) -> list:
    """Конвертирование временного интервала tp с частатой в массив в секундах

    Args:
        tp (np.array): Временной промежуток с частотой дескритезации dt
        dt (float, optional): Частота дисретизации. Defaults to 0.01.
