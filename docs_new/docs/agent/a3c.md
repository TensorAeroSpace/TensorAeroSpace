...
Алгоритм Asynchronous Advantage Actor Critic использует advantage функцию для обновления агентов. Также в данном алгоритме используются асинхронные агенты для исследования среды и в качестве замены реплей буфера.
