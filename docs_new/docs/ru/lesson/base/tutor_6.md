...
Решение этой проблемы – построить **наблюдатель состояния** (state observer) или **оценщик состояния** (state estimator). Это программная или аппаратная модель, которая вычисляет **оценку** вектора состояния, обозначаемую как `✰` (x с крышкой).
