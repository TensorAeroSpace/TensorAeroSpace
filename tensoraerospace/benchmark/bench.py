...
class ControlBenchmark:
    ...
    def becnchmarking_one_step(self, control_signal: np.ndarray, system_signal: np.ndarray, signal_val: float, dt: float) -> dict:
        ...
        if cnt_time is not None and cnt_time < len(control_signal):
            ...
            static_err = static_error(control_signal[start_idx:], system_signal[start_idx:])
    ...
    def plot(self, control_signal: np.ndarray, system_signal: np.ndarray, signal_val: float, dt: float, tps: np.ndarray, figsize: tuple = (1600, 1000), title: str = "Анализ системы управления"):
        """
        ...
        """
        # Получаем метрики
        metrics = self.becnchmarking_one_step(control_signal, system_signal, signal_val, dt)
        control_signal_step, system_signal_step = find_step_function(control_signal, system_signal, signal_val=signal_val)
        ...
        fig = make_subplots(
            ...
            subplot_titles=("Переходный процесс", "Метрики качества", "Ошибка регулирования"),
        ...
        # Время установления
        if metrics['settling_time'] is not None and metrics['settling_time'] <= ntime[-1]:
            fig.add_shape(
                type="line",
                x0=metrics['settling_time'], x1=metrics['settling_time'],
                y0=min(system_signal), y1=max(system_signal),
        ...
        table_data = [
            ...
            ['Время установления', f'{metrics["settling_time"]:.3f}с' if metrics['settling_time'] else 'N/A'],
            ['Время нарастания', f'{metrics["rise_time"]:.3f}с' if metrics['rise_time'] else 'N/A'],
            ['Время пика', f'{metrics["peak_time"]:.3f}с' if metrics['peak_time'] else 'N/A'],
        ...
        # Настройка осей
        fig.update_xaxes(title_text="Время, с", row=1, col=1, showgrid=True, gridwidth=1, gridcolor='lightgray')
        fig.update_yaxes(title_text="Амплитуда", row=1, col=1, showgrid=True, gridwidth=1, gridcolor='lightgray')
        fig.update_xaxes(title_text="Время, с", row=2, col=1, showgrid=True, gridwidth=1, gridcolor='lightgray')
        fig.update_yaxes(title_text="Ошибка", row=2, col=1, showgrid=True, gridwidth=1, gridcolor='lightgray')
    ...
    def compare_systems(self, systems_data: Dict[str, Dict], signal_val: float, dt: float, figsize: tuple = (1800, 1200)):
        """
        ...
        fig = make_subplots(
            ...
            specs=[[{"type": "scatter"}, {"type": "table"}],
                   [{"type": "scatter", "colspan": 2}, None]],
            subplot_titles=("Сравнение систем управления", "Сравнительная таблица", "Сравнение ошибок регулирования"),
            vertical_spacing=0.08,
            horizontal_spacing=0.05
        )
        ...
        control_step, _ = find_step_function(
            first_system['control_signal'], first_system['system_signal'], signal_val=signal_val
        )
        ntime_ref = first_system['time'][:len(control_step)]
        
        fig.add_trace(
            go.Scatter(
                x=ntime_ref, y=control_step,
                mode='lines',
                name='Задающий сигнал',
        ...
        for i, (system_name, data) in enumerate(systems_data.items()):
            ...
            if metrics['settling_time'] is not None and metrics['settling_time'] <= ntime[-1]:
                fig.add_shape(
                    type="line",
                    x0=metrics['settling_time'], x1=metrics['settling_time'],
                    y0=min(system_step), y1=max(system_step),
        ...
        table_data = []
        headers = ['Система', 'Перерег.%', 'Время уст.', 'Время нар.', 'Колеб.', 'IAE', 'Индекс кач.']
        
        for system_name, metrics in all_metrics.items():
            row = [
                system_name[:8] + '...' if len(system_name) > 8 else system_name,
                f'{metrics["overshoot"]:.1f}',
                f'{metrics["settling_time"]:.2f}' if metrics['settling_time'] else 'N/A',
        ...
        fig.update_xaxes(title_text="Время, с", row=1, col=1, showgrid=True, gridwidth=0.5, gridcolor='lightgray')
        fig.update_yaxes(title_text="Амплитуда", row=1, col=1, showgrid=True, gridwidth=0.5, gridcolor='lightgray')
        
        fig.update_xaxes(title_text="Время, с", row=2, col=1, showgrid=True, gridwidth=0.5, gridcolor='lightgray')
        fig.update_yaxes(title_text="Ошибка регулирования", row=2, col=1, showgrid=True, gridwidth=0.5, gridcolor='lightgray')
    ...
    def generate_report(self, control_signal: np.ndarray, system_signal: np.ndarray, 
                       signal_val: float, dt: float, system_name: str = "Система управления") -> str:
        """
        ...
        settling_time_str = f"{metrics['settling_time']:>8.3f} с" if metrics['settling_time'] is not None else "     N/A"
        rise_time_str = f"{metrics['rise_time']:>8.3f} с" if metrics['rise_time'] is not None else "     N/A"
        peak_time_str = f"{metrics['peak_time']:>8.3f} с" if metrics['peak_time'] is not None else "     N/A"
...
║                                                              ║"""
        ...
        overshoot_rating = "Отлично" if metrics['overshoot'] < 5 else "Хорошо" if metrics['overshoot'] < 15 else "Удовлетворительно" if metrics['overshoot'] < 25 else "Плохо"
        settling_rating = "Быстро" if metrics['settling_time'] and metrics['settling_time'] < 2 else "Средне" if metrics['settling_time'] and metrics['settling_time'] < 5 else "Медленно"
        error_rating = "Минимальная" if abs(metrics['static_error']) < 0.01 else "Малая" if abs(metrics['static_error']) < 0.05 else "Значительная"
