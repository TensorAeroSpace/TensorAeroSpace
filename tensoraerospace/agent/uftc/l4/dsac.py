...
class DSACOuter:
    ...
    def __init__(self, cfg: DSACConfig, device: str = "cpu") -> None:
        ...
        actor_cfg = ActorConfig(
            n_state=cfg.n_state, n_action=cfg.n_action, hidden_sizes=cfg.actor_hidden
        )
        critic_cfg = CriticConfig(
            n_state=cfg.n_state,
            n_action=cfg.n_action,
    ...
    def predict(
        ...
        x = torch.tensor(
            np.asarray(x_obs, dtype=np.float32), dtype=torch.float32, device=self.device
        ).reshape(1, -1)
        with torch.no_grad():
        ...
        if self._hold_mode:
            ...
            r_tilde = (
                np.asarray(base_reference, dtype=np.float64).copy()
                + a_np[: int(self.cfg.n_ref_dim)]
            )
        beta_t = risk_gate(z, fdd_severity=fdd.severity, monitor_alarm=monitor_alarm)
    ...
    def _sgd_step(self) -> dict:
        ...
        a = torch.tensor(
            np.stack([t.a_actual for t in batch]),
            dtype=torch.float32,
            device=self.device,
        )
        r = torch.tensor(
        ...
        s2 = torch.tensor(
            np.stack([t.s_next for t in batch]), dtype=torch.float32, device=self.device
        )
        done = torch.tensor(
        ...
        with torch.no_grad():
            ...
            y = r.unsqueeze(-1) + self.cfg.gamma * (1.0 - done.unsqueeze(-1)) * (
                z2 - self.log_alpha.exp() * logp2.unsqueeze(-1)
            )

        z1 = self.critic1(s, a)
        z2_pred = self.critic2(s, a)
        loss_c1 = (
            qr_huber_loss(z1, y, self.cfg.huber_kappa).unsqueeze(0) * weights
        ).mean()
        loss_c2 = (
            qr_huber_loss(z2_pred, y, self.cfg.huber_kappa).unsqueeze(0) * weights
        ).mean()
        ...
        return {
            "loss_c1": float(loss_c1),
            "loss_c2": float(loss_c2),
            "loss_a": float(loss_a),
    ...
    def save(self, dir_path: str | Path) -> None:
        ...
        (dir_path / "dsac_config.json").write_text(
            json.dumps(asdict(self.cfg), indent=2)
        )

    @classmethod
    def from_pretrained(
        cls, dir_path: str | Path, *, cfg: DSACConfig | None = None
    ) -> "DSACOuter":
        dir_path = Path(dir_path)
        if cfg is None:
            import json
        ...
        m.critic1.load_state_dict(
            torch.load(dir_path / "critic1.pt", map_location="cpu")
        )
        m.critic2.load_state_dict(
            torch.load(dir_path / "critic2.pt", map_location="cpu")
        )
        m.target1.load_state_dict(
            torch.load(dir_path / "target1.pt", map_location="cpu")
        )
        m.target2.load_state_dict(
            torch.load(dir_path / "target2.pt", map_location="cpu")
        )
        m.log_alpha = torch.tensor(
            torch.load(dir_path / "log_alpha.pt"), requires_grad=True, device=m.device
        )
