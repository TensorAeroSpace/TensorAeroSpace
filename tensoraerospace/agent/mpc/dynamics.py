...
class DynamicsNN:
    ...
    def generate_training_data(self,num_samples=1000000, state_dim=4, control_dim=1,
                            state_ranges=None, control_ranges=None,
                            A=None, B=None, control_limits=(-25, 25),
        ...
        if A.shape != (state_dim, state_dim):
            raise ValueError(f"Matrix A should have shape ({state_dim}, {state_dim}), but got {A.shape}")
        if B.shape != (state_dim, control_dim):
            raise ValueError(f"Matrix B should have shape ({state_dim}, {control_dim}), but got {B.shape}")
        if len(state_ranges) != state_dim:
            raise ValueError(f"State ranges should have length {state_dim}, but got {len(state_ranges)}")
        ...
        available_signals = {
            ...
            "step": lambda t, n, steps=torch.tensor([-1.0, -0.5, 0.0, 0.5, 1.0, -0.7, 0.3, 0.8]): torch.tensor([steps[i % len(steps)] for i in range(n)]).unsqueeze(-1),
            ...
            "sawtooth": lambda t, n: (torch.remainder(torch.linspace(0, 4, n), 2) - 1).unsqueeze(-1),
            "damped_sine": lambda t, n: (torch.exp(-t / 5) * torch.sin(t)).unsqueeze(-1),
            "chirp": lambda t, n: (torch.sin(t * t / 5)).unsqueeze(-1),
            "modulated_sine": lambda t, n: ((1 + 0.5 * torch.sin(0.5 * t)) * torch.sin(2 * t)).unsqueeze(-1),
            "gaussian_pulse": lambda t, n: (torch.exp(-((t - 5 * np.pi) ** 2) / (2 * (np.pi) ** 2))).unsqueeze(-1),
            "square_wave": lambda t, n: torch.sign(torch.sin(t)).unsqueeze(-1),
            "triangle_wave": lambda t, n: (2 * (torch.remainder(torch.linspace(0, 2, n), 1) - 0.5)).unsqueeze(-1),
            "two_tone": lambda t, n: (0.5 * torch.sin(t) + 0.5 * torch.sin(3 * t)).unsqueeze(-1),
            "random_walk": lambda t, n: torch.cumsum(torch.randn(n) * 0.1, dim=0).unsqueeze(-1),
            "exponential": lambda t, n: (torch.exp(torch.linspace(-2, 2, n)) - torch.exp(torch.tensor(-2.))).unsqueeze(-1),
        ...
        for i in range(state_dim):
            states[:, i] = torch.linspace(state_ranges[i][0], state_ranges[i][1], num_samples) + torch.randn(num_samples) * 0.1
        ...
        if n_signals > 0:
            ...
            for i, (signal_name, signal_func) in enumerate(selected_signals.items()):
                start_idx = i * n
                end_idx = (i + 1) * n if i < n_signals - 1 else num_samples  # Ensure all samples are used
                ...
                if signal.ndim == 1:
                    signal = signal.unsqueeze(-1)  # Add a dimension to make it [n_local, 1]

                controls[start_idx:end_idx, :] = signal

            # Apply control limits
            controls = torch.clamp(controls, min=control_limits[0], max=control_limits[1])

        else:
            # Handle case with custom control ranges
            if len(control_ranges) != control_dim:
                raise ValueError(f"Control ranges should have length {control_dim}, but got {len(control_ranges)}")
            for i in range(control_dim):
                controls[:, i] = torch.linspace(control_ranges[i][0], control_ranges[i][1], num_samples)
                # Apply control limits
                controls[:, i] = torch.clamp(controls[:, i], min=control_limits[0], max=control_limits[1])
    ...
    def train_and_validate(self, states, controls, next_states, epochs=5, batch_size=1024, val_split=0.2, verbose_epoch=10):
        """
        ...
        train_next_states, val_next_states = next_states[:-val_size], next_states[-val_size:]
        print("Подготовка данных")
        train_dataset = torch.utils.data.TensorDataset(train_states, train_controls, train_next_states)
        val_dataset = torch.utils.data.TensorDataset(val_states, val_controls, val_next_states)
        ...
        for epoch in tqdm(range(epochs)):
            ...
            for batch_states, batch_controls, batch_next_states in train_loader:
                ...
                predictions = self.model(torch.cat([batch_states, batch_controls], dim=-1) )
