...
class SAC(BaseRLModel):
    ...
    def __load(
        ...
        # Загружаем модели
        new_agent.critic = torch.load(critic_path, map_location=new_agent.device)
        new_agent.policy = torch.load(policy_path, map_location=new_agent.device)
        new_agent.critic_target = torch.load(
            critic_target_path, map_location=new_agent.device
        )
        ...
        if (
            ...
            loaded_alpha = torch.load(log_alpha_path, map_location=new_agent.device)
        ...
        if load_gradients:
            if policy_optim_path.exists():
                state = torch.load(policy_optim_path, map_location=new_agent.device)
                new_agent.policy_optim.load_state_dict(state)
            if critic_optim_path.exists():
                state = torch.load(critic_optim_path, map_location=new_agent.device)
                new_agent.critic_optim.load_state_dict(state)
            if (
                getattr(new_agent, "automatic_entropy_tuning", False)
                and alpha_optim_path.exists()
            ):
                state = torch.load(alpha_optim_path, map_location=new_agent.device)
