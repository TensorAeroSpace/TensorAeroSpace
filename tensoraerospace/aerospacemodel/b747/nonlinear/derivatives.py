...
def _interp_along(row: list[tuple[float, int]], mach: float, attr: str) -> float:
    ...
    return getattr(
        get_longitudinal(fcs[-1]), attr, getattr(get_lateral(fcs[-1]), attr, 0.0)
    )
...
def cruise_longitudinal_at(altitude_ft: float, mach: float) -> LongitudinalDerivatives:
...
def cruise_lateral_at(altitude_ft: float, mach: float) -> LateralDirectionalDerivatives:
