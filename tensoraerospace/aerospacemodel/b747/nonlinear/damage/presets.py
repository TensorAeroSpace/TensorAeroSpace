...
ELEVATOR_50PCT_LOSS = DamageProfile(
    events=[
        SurfaceEffectivenessEvent(
            trigger_time=5.0,
            surface="elevator",
...
ELEVATOR_JAMMED_NOSE_UP = DamageProfile(
    events=[
        SurfaceJamEvent(
            trigger_time=10.0,
            surface="elevator",
            jam_value=-0.0349,  # -2 deg
...
AILERON_TOTAL_LOSS = DamageProfile(
    events=[
        SurfaceEffectivenessEvent(
            trigger_time=8.0,
            surface="aileron",
...
RUDDER_HYDRAULIC_LEAK = DamageProfile(
    events=[
        SurfaceEffectivenessDecay(
            trigger_time=2.0,
            surface="rudder",
...
LEFT_OUTER_ENGINE_FAILURE = DamageProfile(
    events=[
        EngineFailureEvent(
            trigger_time=10.0,
            engine_id=1,
...
LEFT_TWO_ENGINES_OUT = DamageProfile(
    events=[
        EngineFailureEvent(
            trigger_time=10.0,
            engine_id=1,
...
FLAPS_JAMMED_LANDING = DamageProfile(
    events=[
        FlapJamEvent(
            trigger_time=5.0,
            jammed_config=B747Configuration.LANDING,
...
FLAPS_JAMMED_RETRACTED = DamageProfile(
    events=[
        FlapJamEvent(
            trigger_time=5.0,
            jammed_config=B747Configuration.NOMINAL,
