...
!!! note "Координаты и выходы критика"
    ...
    `J` и `lambda` — независимые выходы сети с общими скрытыми слоями. Актор дифференцирует скалярный выход `J`. Равенство `lambda == dJ/dy` не обеспечивается; штраф за изменение управления является регуляризацией актора и не входит в непосредственную стоимость критика.
