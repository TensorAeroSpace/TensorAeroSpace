...
IMGDHP — инкрементальный модельно-ориентированный вариант Global Dual Heuristic Programming из семейства Adaptive Critic Designs (ACD). Предназначен для онлайн-адаптивного управления нелинейными объектами в условиях частичной наблюдаемости. Агент объединяет рекурсивный метод наименьших квадратов (RLS) для идентификации объекта с двуглавым критиком, оценивающим функцию стоимости \(J\) и отдельное приближение её производной — вектор сопряжённых переменных \(\lambda\). Актор использует обученную оценку производной \(\lambda\) для градиента будущей стоимости; независимая голова \(J\) задаёт скалярное значение в отчёте. См. также нелинейную модель F-16: [NonlinearLongitudinalF16](../model/f16_nonlinear_longitudinal.md).
...
- **Двойной критик GDHP**: критик выдаёт как \(J(o)\) (скалярная стоимость), так и \(\lambda(o)\) (вектор сопряжённых переменных), обучая общие скрытые слои по двум функциям потерь
- **Модельно-предиктивное обновление актора**: модель использует идентифицированные матрицы \(A\), \(B\); градиент по управлению проходит через \(B\)
...
Сравнение реализаций в этом репозитории; HDP — режим `ADP(design="hdp")`.

| Аспект | HDP | IHDP | **IMGDHP** |
| --- | --- | --- | --- |
| Идентификация | Известная линейная модель | МНК по окну приращений | Онлайн RLS (инкрементальная линейная) |
| Выход критика | \(J(o)\) | \(J(o)\) | \(J(o)\) + \(\lambda(o)\) (двойной) |
| Обновление актора | По известной модели | По идентифицированной модели | По прогнозу модели, градиент через \(B\) |
| Вход | Состояние и задание тангажа | Выбранные состояния и задание | Наблюдения, задание и ошибка слежения |
| Сети / идентификация | PyTorch / известные матрицы | PyTorch / NumPy | PyTorch / NumPy |
...
1. **Расширение наблюдения**: \(o_t = [y_t;\; r_t;\; e_t]\), где \(e_t = y_t[\text{tracking}] - r_t\) при единичном `obs_scale`; иначе наблюдения, задание и ошибки масштабируются по своим каналам
2. **Актор формирует действие**: \(u_t = \pi_\theta(o_t)\)
3. **Исполнение** \(u_t\) в среде, получение \(y_{t+1}\)
4. **Одношаговая стоимость**: \(c_t = e_t^\top Q e_t\)
5. **Обновление RLS** (при \(t \geq 1\)): по данным \((y_{t-1}, y_t, y_{t+1}, u_{t-1}, u_t)\) получаем \(A_t\), \(B_t\)
...
\[
\min_\theta \; c(\hat{y}_{t+1}, r_{t+1}) + \gamma \, J\!\left(\hat{o}_{t+1}\right) + \rho \|u_t-u_{t-1}\|^2, \quad \text{градиент проходит через } B_t
\]

!!! note "Координаты и выходы критика"
    RLS и прогноз модели используют физические наблюдения. `obs_scale` однократно масштабирует входы сетей и ошибки слежения, отдельно для каждого отслеживаемого канала. Актор и критик используют масштабированную стоимость слежения; целевая производная учитывает множители масштаба по правилу цепочки.

    `J` и `lambda` — независимые выходы сети с общими скрытыми слоями. Вклад будущей стоимости в градиент управления равен `gamma * B.T @ lambda_next` в физических координатах. Равенство `lambda == dJ/dy` не обеспечивается, поэтому градиент независимой головы `J` не подменяет обученную оценку производной; штраф за изменение управления является регуляризацией актора и не входит в непосредственную стоимость критика.
...
`save(path, save_gradients=True)` сохраняет сети, оптимизаторы, RLS, счётчик прогрева, историю текущего перехода и состояние генератора шума исследования. Загружайте через `IMGDHPAgent.from_pretrained(path, load_gradients=True)`, чтобы продолжить обучение. Для повторения траектории нужно также восстановить состояние среды, задание и индекс шага. При старте нового эпизода вызовите `agent.reset()`; обученные веса, RLS и общий счётчик обучения сохранятся.

Старые файлы без `training_state.json` загружаются с пустой историей и нулевым счётчиком прогрева. Шум исследования использует отдельный генератор агента и не зависит от посторонних вызовов `np.random`.
