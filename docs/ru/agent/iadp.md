...
Регулятор обновляет инкрементальную модель онлайн. Ему всё ещё нужны разумная инициализация и информативные данные; адаптация не гарантирует устойчивость при произвольных изменениях объекта или привода. Лётные испытания из статьи относятся к реализации её авторов, а не к этой библиотеке. См. [NonlinearLongitudinalF16](../model/f16_nonlinear_longitudinal.md).
...
В уравнении (10) и на рис. 2 функция ценности использует прогноз инкрементальной
модели: `Xnext = X + F @ dX + G @ du`. Измеренное следующее состояние обучает RLS;
для критика сохраняется соответствующий прогноз модели. У первого перехода
после reset остаётся измеренная цель: предыдущее приращение ещё неизвестно.

Пакетная задача решается непосредственно через SVD, без ухудшения обусловленности
из-за нормальных уравнений. Ридж к нулю сохранён; ридж, PSD-проекция и сглаживание —
расширения библиотеки относительно псевдообратной матрицы в статье. По неинформативному
окну нельзя определить все коэффициенты P. Сохранение прежней P в ненаблюдаемых
направлениях потребовало бы другой целевой функции.

В статье используется постоянное возбуждение и разные частоты обновления модели
и политики. `policy_eval_every=50` при `dt=0.01` означает **2 Гц**, а не 20 Гц.
Окно, забывание и дисконтирование заданы на отсчёт и требуют пересмотра при смене dt.
При численном переполнении RLS выдаёт ошибку до сохранения неверных параметров;
это не обеспечивает ограниченность ковариации без достаточного возбуждения.

## Фактическое управление и инициализация

Если привод ограничивает или задерживает команду, передавайте фактический вход
через `learn(next_state, reference, k, applied_action=actual_input)`.
Единицы и триммерное смещение должны совпадать с `predict()`; без обратной связи
предполагается точное выполнение команды. Фактический вход используется в RLS,
стоимости управления и следующем приращении. Линейные среды B747/LAPAN возвращают
его в `info["applied_action"]` в градусах.

Первый переход после reset сохраняется для обучения функции ценности, но RLS
ждёт два последовательных перехода. Неизвестное предыдущее приращение состояния
не подменяется нулём. Начальное возбуждение соблюдает те же ограничения амплитуды
и скорости в единицах управления в секунду, что и обученная политика.

В течение `model_learning_only_steps` обучается только модель: матрица критика
и его буфер не меняются. Буфер заполняется последующими переходами замкнутого
контура. RLS продолжает адаптацию и после этой фазы; одного этого параметра
недостаточно для полного последовательного режима статьи с замороженной моделью.
...
cfg = IADPConfig(
    ...
    policy_eval_every=50,                       # 2 Гц при dt = 0.01 с; для 20 Гц нужно 5
...
!!! tip "Warm-start `P_init` по DARE для быстрой сходимости"
    Онлайн-МНК подбирает \(\tilde{P}\) на окнах переходов. Конечное окно, масштабы признаков и регуляризация могут существенно ухудшать оценку; общего ограничения потери качества в несколько процентов нет. Засейте `P_init` аналитическим LQT-DARE, вычисленным по warm-start модели:
...
| `policy_eval_regularization` | 1e-4 | Ридж к нулю; решение через SVD |
...
## Предпосылки стоимости и проверка обучения

По формулам (3) и (5) оригинальной статьи `0 < gamma < 1`, а `Q` и `R` должны быть конечными симметричными положительно полуопределёнными матрицами. Конструктор отклоняет нарушения этих условий. Нулевые веса отдельных состояний допустимы.

`policy_eval_blend=0` полностью пропускает решение МНК и сохраняет матрицу критика; идентификация модели продолжает работать. Это диагностический режим библиотеки.

Число строк окна само по себе не гарантирует идентифицируемость: у квадратичных признаков дублируются смешанные произведения. Для `d` активных переменных максимум независимых признаков равен `d*(d+1)/2`. При пяти активных переменных ранг 15 нормален; дополнительно нужно оценивать масштабы и информативность данных.

Ридж к нулю зависит от единиц и масштаба состояний. Он меняет задачу из статьи и не гарантирует сохранение хорошей исходной политики. Для проверки вычислений используйте `scripts/validate_iadp_lapan_critic.py`: он сравнивает один шаг оценки политики с решениями Риккати и Ляпунова на данных точно известного регулятора. Это проверка алгебры, а не сходимости обучения в полёте.


### Отдельное ожидание данных критика после reset

`policy_eval_warmup_updates` относится к накопленному числу обновлений RLS и сохраняется между эпизодами. Буфер критика при `reset()` очищается. Чтобы после сброса дождаться нового окна, задайте `policy_eval_min_samples=policy_eval_window`. При `None` сохраняется прежний порог `max(n_aug**2, 4)`; старые checkpoint загружаются с этим поведением. Новый параметр сохраняется вместе с checkpoint и не меняет уравнения обучения.

В полётной статье Konatala et al. (раздел IV, стр. 11) для описанного последовательного опыта использовали собранное 20-секундное окно. Число отсчётов и фазы в конкретной симуляции требуют отдельной настройки. Ожидание окна не доказывает сходимость или достаточную информативность данных.

В проверочном скрипте `scripts/validate_adaptive_tracking.py` режим `--reference-mode oscillator --full-state` предоставляет четыре состояния генератора двух синусоид; только желаемая q входит в стоимость. Это дополнительная информация о reference, а не воспроизведение неизвестного пилотского задания. Само значение синусоиды не определяет её последующий ход без информации о фазе. Режим нужен для отделения неполного описания reference от ошибок реализации агента.
