...
## Индексация решения и численные ошибки

Пошаговая стоимость реализации использует предсказанное состояние `x[i+1]`.
Задание из `horizon` строк относится к `x[1]` … `x[N]`; вариант из `horizon+1`
строк дополнительно содержит задание для исходного состояния. Терминальный
штраф добавляется к стоимости последнего шага. `final_cost` соответствует
возвращённой последовательности; при `track_best=True` проверяется и результат
последнего обновления оптимизатора.

Передавайте фактическое предыдущее управление в `u_prev`, чтобы ограничить первый
шаг. Без него ограничения скорости действуют между последующими командами,
а начальная позиция исполнительного органа неизвестна. Ограничения должны быть
совместимы. Неконечная стоимость или градиент вызывает `RuntimeError`; резервный
регулятор не включается автоматически. Конечность чисел и терминальный вес сами
по себе не гарантируют устойчивость замкнутого контура.
