...
## Переходы, оценка и продолжение обучения

DSAC ожидает нормализованные действия в `[-1, 1]`. Для среды с действиями в физических единицах нужна обёртка нормализации.

- Replay хранит копию наблюдения до шага, в том числе для сред с повторно используемым буфером наблюдений.
- Истинное завершение обнуляет продолжение целевой ценности. Ограничение времени сохраняет его, если доступно последнее наблюдение. При автоматическом сбросе векторной среды используются `final_observation` и необязательная маска его наличия. Если финальное наблюдение отсутствует, продолжение консервативно обнуляется, чтобы не смешивать эпизоды.
- `select_action(..., evaluate=True)` и пакетный вариант возвращают среднее действие без исследовательского шума и расходования случайных чисел Torch.
- Повторные вызовы скалярного `train()` сохраняют `total_env_steps` и `total_updates`: прогрев и расписание целевых обновлений продолжаются. Возвращаемые `total_steps` и `updates` относятся только к текущему вызову. Векторное логирование считает отдельные переходы; `train_vector(warmup_steps=...)` по-прежнему явно задаёт прогрев в начале каждого вызова.
- Checkpoint сохраняет оба счётчика. При загрузке старого формата они равны нулю. Replay, состояние среды и генераторов случайных чисел не сохраняются, поэтому загрузка не воспроизводит точное продолжение случайной траектории. Для состояния оптимизатора нужны `save_gradients=True` и `load_gradients=True`.
