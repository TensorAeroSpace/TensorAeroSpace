...
## Инициализация, replay и продолжение обучения

DQN и PER-NARX начинают с независимой копии весов основной сети в целевой, включая отложенную инициализацию входного слоя. Загрузка checkpoint DQN сохраняет записанную отстающую целевую сеть.

Replay владеет копиями наблюдений. Сборщик записывает состояние до шага и использует `final_observation` или `terminal_observation`, если они переданы на границе сброса. Истинное завершение обнуляет продолжение целевой ценности; тайм-аут сохраняет его.

Повторные вызовы `train()` сохраняют счётчики, прогрев replay и фазу обновления целевой сети. Каждый вызов по-прежнему сбрасывает среду и задаёт бюджет переходов: разрезание незавершённого эпизода не эквивалентно непрерывному сбору. Checkpoint DQN сохраняет счётчики шагов, обновлений и эпизодов. В старых checkpoint они равны нулю. Replay, состояние среды и RNG не сохраняются, поэтому загрузка не гарантирует точного продолжения случайной траектории.

**Исследование управляется явно:** `train()` сохраняет `epsilon`. Вызывайте `e_decay()` по выбранному расписанию; метод учитывает `min_epsilon` и сохраняет явно отключённое исследование `epsilon=0`. Автоматическое снижение меняет распределение данных и может ухудшать политику. `verbose=False` отключает вывод прогресса.
