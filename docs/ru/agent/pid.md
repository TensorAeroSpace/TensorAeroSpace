...
Когда управляющий выход насыщается (достигает пределов исполнительного механизма), интегральная составляющая может "накручиваться", вызывая большое перерегулирование. Наша реализация включает **условное интегрирование (anti-windup)**: интегрирование блокируется только тогда, когда его вклад усиливает насыщение. Изменения, возвращающие управление в допустимый диапазон, разрешены, в том числе при отрицательном `ki` и несимметричных границах действий.
