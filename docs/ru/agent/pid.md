...
## Измеренная производная и ограничения привода

`PID(..., output_limits=(-8, 8), rate_limit=20, dt=0.02)` поддерживает явные
ограничения амплитуды и скорости управления в секунду. Если доступны производная
измерения и фактический предыдущий выход, вызывайте
`select_action(setpoint, measurement, measurement_rate=rate, applied_output=previous_actual)`.
Условное интегрирование защищает от накопления интеграла при обоих ограничениях.
Без этих необязательных аргументов сохраняются разностная производная и
ограничения среды. Соблюдайте согласованность единиц измерения и коэффициентов.

`tensoraerospace.agent.pid.LateralAircraftPID` использует два экземпляра `PID`
для крена и курса B747, переводит угловые скорости корпуса в производные углов
Эйлера, оборачивает ошибку курса и возвращает элероны/руль направления в
**градусах**. `B747LongitudinalHold` возвращает абсолютный угол руля высоты в
**радианах** и нормированный газ. Настройки по умолчанию относятся к указанному
крейсерскому сравнению; для другой балансировки их нужно проверить заново.

`tensoraerospace.agent.lqr.LQRAgent(A, B, Q, R)` решает дискретную задачу Риккати
и возвращает `-K @ state` через `predict`. Передавайте отклонения от корректной
балансировки; класс не добавляет балансировочный вход и ограничения привода.
Для LQI передаются расширенные матрицы и интегралы измеренной ошибки.
Общие ограничения показаны в [протоколе B747](../cookbook/09_fault_tolerance.md).
