...
## Корректность переходов и скалярного управления

Каждое действие в пакете связывается со своей оценкой преимущества. Логарифм
совместной вероятности действия и энтропия имеют размер `(batch,)`, включая
единственный управляющий канал. Умножение `(batch, 1)` на `(batch,)` смешивало
разные переходы; исправление действует и в `Net.loss_func`, и в обновлении worker.

Worker копирует наблюдение до `env.step()`. При автоматическом reset продолжение
ценности оценивается по `final_observation` или `terminal_observation`.
Ограничение времени Gymnasium и старый `TimeLimit.truncated` сохраняют bootstrap;
истинное завершение его отключает. Интервал обновления учитывает каждый переход,
включая последний шаг эпизода. В пакет сохраняется исходное случайное действие,
а среде передаётся его ограниченная версия.

Исправления меняют траекторию обучения и количество обновлений. Конечные loss
и успешные тесты не гарантируют улучшения качества или сходимости: сравнивайте
фиксированные seed и сохранённые политики на нужном объекте. Проверка одного
worker не подтверждает воспроизводимость асинхронного исполнения.
