# Рецепт 11 — IHDP на нелинейном F16


Градиент актора учитывает физический масштаб выхода. После исправления и подбора используются `learning_rate=2/15*2`, `learning_rate_min=0.001/15*2`. Результаты включают упреждение по обратной модели и не относятся к IHDP без этого дополнения.
...
![Feedforward trim-кривая](../example/agent/ihdp/img/ihdp_nl_ff_curve.png)
...
![Задающий сигнал](../example/agent/ihdp/img/ihdp_nl_reference.png)
...
- **`NN_initial = 47`** — фиксированная инициализация для сравнения, а не гарантия робастности.
...
![Слежение IHDP + FF](../example/agent/ihdp/img/ihdp_nl_tracking.png)

Метрики на второй половине эпизода (40 с → 80 с):

| Метрика | Значение |
|---------|----------|
| Средняя абсолютная ошибка | **0.0744°** (2.48 % от амплитуды) |
| RMS | 0.1040° |
| Макс. абсолютная ошибка | 0.2493° (8.31 % от амплитуды) |
...
- **Инициализация.** Перед выводами о робастности повторите опыт с другими seed. Прежний перебор использовал старый масштаб градиента.
...
[Алгоритм и перенос параметров](../agent/ihdp.md) · [Рецепт IM-GDHP](12_imgdhp.md)
