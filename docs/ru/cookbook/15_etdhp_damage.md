...
- Корректно ли реагирует Лишпицев event-trigger на изменившуюся динамику?
...
Онлайн-обучение actor/critic остаётся включённым в обоих эпизодах оценки, так что триггер срабатывает каждый раз, когда ошибка слежения превышает Лишпицев порог. Plant-NN **заморожена** на оффлайн-предобученных весах — это и есть структурное ограничение, которое мы увидим ниже.
...
- **Лишпицев event-trigger корректно реагирует**: число триггеров после t = 20 с **удваивается** относительно baseline (547 vs 261). Супервизор работает — но не может выдать actor/critic стабильную post-damage политику, потому что якобианы plant-NN `F = ∂f/∂x`, `G = ∂f/∂u` больше не соответствуют повреждённой динамике.
