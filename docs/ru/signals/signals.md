...
!!! note "Временная сетка и частотная развёртка"
    Дробные амплитуды сохраняются и при целочисленном массиве времени. `chirp()` начинает с нулевой фазы и частоты `f0` в `tp[0]`, достигая `f1` в `tp[-1]`; перенос начала времени сохраняет форму сигнала. Экспоненциальная развёртка требует положительных граничных частот и устойчива при почти равных частотах.
