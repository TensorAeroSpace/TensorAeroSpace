...
`trim(h, V)` обнуляет все шесть ускорений в связанных осях при нулевых
угловых скоростях. Искомые величины: угол атаки, скольжение, крен,
элеватор, элерон и газ. Тангаж вычисляется из условия нулевой вертикальной
скорости. `converged` также проверяет диапазон газа и отклонение обоих элевонов.

При высоте 178 м и скорости 18 м/с решение в неподвижном воздухе:
α≈7,62°, β≈0,59°, крен≈−0,63°, δ_e≈−1,96°, δ_a≈−3,31°, δ_T≈0,641.
Это равновесие реализованной модели. Оно не воспроизводит точно режим
из статьи: в эксперименте присутствовал ветер, а в пакете используется
упрощённая модель двигателя. См. [первоисточник, уравнения 36–38](https://link.springer.com/article/10.1007/s13272-025-00816-3).

Используйте возвращённое состояние и **все три** балансировочные команды:
...
x0 = result.to_state()
u0 = result.to_control()  # [элеватор, элерон, газ]
```

Нулевой элерон нарушает боковую балансировку. Необязательный аргумент
`initial_guess=(alpha, elevator, throttle)` сохраняет прежний формат.
...
* Команды ограничиваются диапазоном действий, затем смешиваются с ограничением
  ±20° для каждого физического элевона. NaN и бесконечности отклоняются.
...
* **Повреждения и обледенение**: Изменение аэродинамики при повреждениях пока
  не реализовано. Передача `damage_profile` или `damage_event_callback` в среду
  вызывает `NotImplementedError`, чтобы сценарий не выполнялся с исправным объектом.
