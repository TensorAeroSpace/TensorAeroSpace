...
=== "Переменные"
    ...
    - **δ_t**: безразмерная команда тяги (не влияет на эту редуцированную модель)
...
Состояния — отклонения от балансировочного режима. В источнике используется `-h`; здесь высота направлена вверх, поэтому у последней строки и последнего столбца `A` изменены знаки.

Порядок выходов — `[Va, alpha, theta, q, h]`, где `Va = 0.9985*u + 0.05399*w`, `alpha = -0.003176*u + 0.05874*w`.

!!! note "Ограничение канала тяги"
    В опубликованной редуцированной модели столбец тяги нулевой. API принимает и записывает команду, но она не влияет на траекторию. Для учёта тяги нужна отдельная модель двигателя.
