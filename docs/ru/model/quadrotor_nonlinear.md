...
!!! note "Ограничения роторов и повреждения"
    Команды ограничиваются диапазоном оборотов до применения эффективности `mu`. Отказавший ротор сохраняет нулевую тягу даже при положительных минимальных командных оборотах. События при `t=0` срабатывают на первом шаге. Деградация рассчитывается по `mu_next = mu_floor + (mu - mu_floor)*exp(-dt/tau)`, поэтому крупный шаг не опускает эффективность ниже асимптоты. Методы получения состояния модели возвращают независимые копии.
...
## Время событий и постепенная потеря эффективности

Среда разделяет шаг интегрирования в моменты запланированных и внедрённых
событий. Отказ на правом конце шага меняет тягу следующего интервала и не
действует задним числом. Журнал хранит `trigger_time`. События сортируются по
времени; при совпадении времени сохраняется порядок профиля, затем внедрения.
Износ начинается с момента активации; RK4 вычисляет экспоненциальную
эффективность на каждой стадии внутри интервала.

`RotorDamageManager.update()` вычисляет длительность по `t_current - t_previous`.
Аргумент `dt` сохранён для совместимости. В `info` и истории управления модели
хранятся эффективные команды **на конец шага**, а не средние за интервал со
событием. Модель принимает `control_segments` для кусочно-гладкого управления,
сохраняя один отсчёт состояния на внешний шаг.

Начальное состояние копируется. Состояния, команды, времена событий и постоянные
износа должны быть конечными; шаг времени и пределы скорости роторов проверяются.
Ограничения моторов применяются до повреждений: нижний предел скорости не может
восстановить тягу остановленного ротора.

Аналитические проверки свободного падения и экспоненциального изменения тяги,
а также сравнение с кусочным DOP853 проверяют время событий и интегрирование.
Они не подтверждают параметры сопротивления, моторов и отказов по лётным данным.
