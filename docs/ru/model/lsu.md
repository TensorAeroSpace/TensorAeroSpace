...
-  :math:`q` Угловая скорость Тангажа [рад/с]
-  :math:`\theta` - Тангаж [рад]
-  :math:`\eta` - Угол отклонения стабилизатора [рад]
...
## Единицы, привод и интерфейс среды

Состояние `[u, w, q, theta]` задаёт отклонения от режима полёта в м/с, м/с, рад/с
и рад. `LAPAN.run_step` принимает руль в **радианах**. Коэффициенты A/B совпадают
со страницей 86 работы
[Septiyana et al. (2020)](https://ejournal.brin.go.id/ijoa/article/download/12529/9853).
Это линейная модель; совпадение коэффициентов не заменяет проверку по лётным данным.

Сохранены настроенные ограничения объекта ±40° и 300°/с, действующие с первой
команды. `initial_control` задаёт начальный руль в радианах, по умолчанию ноль;
`initialise_system` восстанавливает его. Некорректные состояния, команды, время
и горизонт отклоняются. `get_output` возвращает все записанные выходы до перехода;
график скорости сохраняет м/с и не меняет исходные данные.

`LinearLongitudinalLAPAN` принимает команды руля в **градусах**, в пределах ±25°,
и переводит их в радианы для модели. Ранее объявленный диапазон ±25 передавался
прямо как радианы. Для существующих контроллеров нужно проверить масштаб команд.
Наблюдения и углы задания остаются в радианах; по умолчанию выдаются `[theta, q]`.
`output_space` определяет порядок и размер наблюдения, а `tracking_states` независимо
выбирает состояния модели для награды. Один канал задания относится к первому
отслеживаемому состоянию, несколько каналов используют среднюю абсолютную ошибку.
Функция задания получает `i*dt` секунд; горизонт возвращает `truncated=True`.
Исходные массивы копируются для воспроизводимого reset.
