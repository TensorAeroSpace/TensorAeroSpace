...
Это значения упрощённой модели двигателя. По одной кривой тяги нельзя
подтвердить продолжительность полёта или соответствие лётным измерениям.

## Trim finder

`tensoraerospace.aerospacemodel.aai_shadow.nonlinear.trim(h, V)` решает
$\dot u = \dot w = \dot q = 0$ методом нелинейных наименьших квадратов
с ограничениями: газ `[0, 1]`, руль высоты `±params.elevator_max_rad`, угол
атаки `(-π/2, π/2)`. Сходимость требует успешного завершения решателя и нормы
остатка не выше `tol`. Полный газ при ненулевом ускорении не считается
установившимся полётом.

На высоте 1000 м при 36 м/с газ составляет около 0,930, при 37,8 м/с —
около 0,982. Ранее во втором режиме неограниченный поиск Ньютона попадал
в насыщение двигателя и застревал. В обеих точках дрейф скорости, углов
и высоты меньше `1e-6` за 5 с при RK4 и `dt=0.01`. Коэффициенты аэродинамики
и модель двигателя не изменены.

Это проверка согласованности внутри модели. Отказ решателя сам по себе
не доказывает реальный потолок или границы лётного диапазона самолёта.
Приводы остаются идеальными; ограничения скорости и смешивание физических
поверхностей V-хвоста требуют отдельной модели приводов.

## Gymnasium env

Перед интегрированием команды ограничиваются заявленными нормализованными
или физическими пределами. Нечисловые действия и некорректные настройки
времени вызывают `ValueError`. Среда копирует `initial_state`. Динамика
повреждений не реализована: заданные `damage_profile` или
`damage_event_callback` вызывают `NotImplementedError`, а не игнорируются.
