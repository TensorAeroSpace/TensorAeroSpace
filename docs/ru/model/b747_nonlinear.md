...
`NonlinearB747(..., damage_profile=profile)` содержит штатный планировщик отказов,
который использует и `NonlinearB747Env`. Интегрирование делится в моменты событий.
Отказ на правой границе шага действует на следующий интервал; отказ в нулевой
момент активен уже при reset. `damage_events_log` хранит заданные моменты событий.
`lateral_state`, `lateral_transition` и `lateral_linearization` предоставляют
локальное боковое представление в градусах, используемое в сравнениях регуляторов.
