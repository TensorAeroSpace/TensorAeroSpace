...
!!! tip "Ограничения привода"
    ...
    - Максимальная величина: \(\pm 20^\circ\)
...
## Договорённости симуляции и совместимость политик

Состояние Python-модели — **отклонения от режима** `[u, w, q, theta]` в м/с,
м/с, рад/с и рад. В `u` нельзя записывать абсолютную крейсерскую скорость.
Гравитационный коэффициент `-9.81` и уравнения этой страницы используют СИ.
Отдельный старый `simulinkModel/f4c/f4c_data.m` содержит другие коэффициенты
в английских единицах; это не эквивалентная реализация Python-модели.
Точное соответствие коэффициентов Python режиму полёта из указанного источника
ещё требует проверки. Совпадение с интегратором не подтверждает реальный самолёт.

`LongitudinalF4C` ограничивает руль по положению ±20° и скорости 60°/с в радианах,
включая первый шаг. `initial_control` задаёт начальное положение в радианах,
по умолчанию ноль; `initialise_system` восстанавливает его. Некорректные состояния,
команды, шаг времени и горизонт отклоняются. `get_output` возвращает все записанные
выходы до перехода; график скорости сохраняет м/с и не меняет исходную историю.

`LinearLongitudinalF4C` принимает команды руля в **градусах**. Порядок наблюдений
по умолчанию `[u, w, q, theta]`; `output_space` выбирает эти состояния и определяет
форму наблюдения. Старые обозначения `alpha`/`V` не соответствовали вычисляемым
величинам и не являются именами состояний. Награда выбирает `tracking_states` из
полного состояния независимо от наблюдений. Один канал задания относится к первому
отслеживаемому состоянию, несколько каналов дают среднюю абсолютную ошибку.
Функция задания получает время в секундах согласно `dt`. Горизонт завершает эпизод
через `truncated`, сохраняя возможность продолжения оценки ценности.

`F4CPitchEnvNormalized` использует **фактическое** положение руля после ограничений
в наблюдении, штрафах энергии и плавности; оно также доступно в `info['elevator_deg']`.
`initial_elevator_deg` задаёт положение физического привода. Параметр
`use_initial_action_on_first_step` по-прежнему определяет подмену первой команды
начальным значением. Эпизод завершается при выходе за ограничения тангажа 30° или
угловой скорости 10°/с. Последнее — настройка среды, а не подтверждённое лётное ограничение.

Сохранённые политики требуют повторной оценки: изменились наблюдение предыдущего
управления, первый переход, награда и завершение по угловой скорости.
Совпадение размеров нейросети не гарантирует совместимость её поведения.
