...
!!! note "Состояния и наблюдения модели"
    Внутренний порядок состояний — `[u, w, theta, q, h]`; выходы — `[Va, alpha, theta, q, h]`. `run_step()` возвращает выходы после перехода, а `store_outputs` хранит выборки до перехода. Имена `u`/`w` сохранены как старые псевдонимы выходов `Va`/`alpha`. Среда `LinearLongitudinalUltrastick` выбирает физические состояния, а `ImprovedUltrastickEnv` нормализует наблюдения текущего шага.

!!! note "Ограничение канала тяги"
    В текущей редуцированной модели столбец тяги нулевой. Второе действие принимается и учитывается в штрафах награды, но не влияет на траекторию. См. [уравнения модели](suav.md#математическая-модель).
...
# Начальное состояние: [u, w, theta, q, h] — полное состояние Ultrastick
...
## Шаг времени и начальное положение руля

`dt` задаёт шаг в секундах одновременно для среды и объекта.
`Ultrastick(initial_control=[elevator_rad, throttle])` задаёт начальное положение
исполнительных органов; по умолчанию оно нулевое. Ограничение скорости действует
с первой команды. Это дискретное ограничение при удержании команды на шаге,
а не непрерывная модель сервопривода.

Улучшенная среда ограничивает начальный руль диапазоном ±15°, тягу — [0, 1] и
передаёт согласованное начальное положение модели. История действий, награда и
`info` используют реально приложенную команду после ограничений объекта.
`use_initial_action_on_first_step` сохраняет начальную команду на первом шаге.
Неконечные команды отклоняются до продвижения модели.

Старая среда `LinearLongitudinalUltrastick` принимает одну команду руля в градусах.
Горизонт времени возвращает `terminated=False, truncated=True`, сохраняя
продолжение оценки ценности. Улучшенная среда также завершает горизонт через
`truncated`; превышение ограничения тангажа вызывает `terminated`.
