...
2. Среда ограничивает \(u_t\) диапазоном \([-1, 1]\), отображает в \(\delta_e\) в радианах и продвигает внутреннюю модель ELV.
...
- Внутреннее представление состояния следует порядку `[w, q, theta]` (единицы СИ: м/с, рад/с, рад), но наблюдения нормализованы в `[-1, 1]`.
- Модель ELV принимает вход руля высоты в радианах; среда выполняет преобразование из нормализованного пространства действий автоматически.

## Литература

- `tensoraerospace/envs/elv.py` -- полная реализация среды
- Модель динамики ракеты-носителя ELV: `tensoraerospace/aerospacemodel/elv.py`
