...
## Единицы и контракт среды

`LongitudinalUAV` принимает руль высоты **в радианах**. Ограничения: ±25° с
пересчётом в радианы и 60°/с, начиная с `initial_control` (по умолчанию ноль),
включая первый шаг. Команда ограничивается между отсчётами и удерживается весь
шаг. Состояние — отклонения `[u, w, q, theta]`: скорости в м/с, углы в рад,
угловая скорость в рад/с. `dt` задаётся в секундах и согласован у модели и среды.

Python возвращает физические состояния (`C=I`). В MATLAB первые два выходных
канала преобразованы в воздушную скорость и угол атаки; для сравнения реализаций
используйте внутренние состояния. `get_output()` включает все записанные
отсчёты до перехода, включая последний; `run_step()` возвращает следующее состояние.

`LinearLongitudinalUAV.action_space` описывает действительные границы в радианах;
раньше ошибочно объявлялось ±60. Политики с нормализацией управления через эти
границы требуют повторной оценки при загрузке. `output_space` выбирает наблюдения
и по умолчанию следует `state_space`. Награда использует `tracking_states` из
полного состояния независимо от порядка наблюдений. Одноканальное задание
относится к первому отслеживаемому состоянию; многоканальное — к соответствующим
состояниям со средней абсолютной ошибкой. Функция задания получает время
в секундах. Горизонт времени возвращает truncation, сохраняя bootstrap.

Четырёхмерная модель не учитывает связи с высотой и двигателем из исходной
модели. Она не заменяет нелинейный симулятор и проверку разных режимов полёта.
