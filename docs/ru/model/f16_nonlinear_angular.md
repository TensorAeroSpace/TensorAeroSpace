...
## Баланс сил и установившийся полёт

Осевой коэффициент `Cx` не равен сопротивлению вдоль скорости. В связанных осях NED сила равна `[T - q*S*Cx, q*S*Cz, -q*S*Cy]`; ускорение вдоль скорости вычисляется скалярным произведением этой силы на `[cos(alpha)*cos(beta), sin(beta), sin(alpha)*cos(beta)]`, делением на массу и добавлением проекции тяжести. При отслеживании высоты и скорости тяга также влияет на угол атаки и скольжение.

`find_trim` совместно определяет угол атаки, стабилизатор и тягу. `converged=True` требует выполнения всех трёх уравнений (`dalpha`, ускорение тангажа, `dV`) с заданной точностью и допустимых ограничений руля и тяги. Искусственная минимальная тяга не вводится; переданные параметры объекта не изменяются. Траектории со старым преобразованием сил следует пересчитать.

### Контракт среды с высотой и управляемой тягой

В `NonlinearAngularF16(track_altitude=True)` начальное состояние может содержать
14 элементов либо 16: последние два — высота в метрах и скорость в м/с.
Для 14 элементов модель добавляет `Oy=3000 м`, `V=120 м/с`. `reset()` и `step()`
возвращают одинаковое число компонент, включая дополнительные наблюдения
повреждений, если они включены.

При `thrust_mode="control"` последняя компонента действия — тяга **в ньютонах**,
с диапазоном `[0, T_max_thrust]` (по умолчанию `[0, 130000]`). Остальные компоненты
среда принимает в градусах: стабилизатор ±25°, элерон ±21,5°, руль направления
±30°; затем переводит в радианы. История
управления модели содержит также ограниченную команду тяги. Исправление диапазона
меняет масштаб действий вновь создаваемой политики с управляемой тягой; старые
веса нельзя автоматически считать обученными для нового масштаба.

Шаг времени и начальная скорость должны быть положительными и конечными.
NaN/Inf в состоянии или управлении отклоняются до изменения динамики.

### Время повреждений и сохранение среды

Шаг интегрирования разбивается в моменты событий повреждения: сначала движение
до события, затем изменение параметров и движение оставшуюся часть шага.
Событие на правой границе влияет только на последующее движение. События в
`trigger_time=0` применяются до наблюдения, возвращаемого `reset()`.
События профиля и добавленные через `inject_event()` выполняются в порядке
времени; каждое событие профиля применяется один раз до следующего сброса.

`damage_events_log` и `damage_state_log` содержат фактическое время события.
Истории модели сохраняют одну запись за внешний шаг; история управления
фиксирует эффективную команду на его конце. Это не среднее управление за шаг,
внутри которого произошёл отказ.

При `track_altitude=True` `thrust_factor` и `hard_failure` уменьшают силу тяги
в уравнениях скорости, угла атаки и скольжения. Сокращённый режим с постоянной
скоростью не моделирует поступательную реакцию на отказ двигателя.

`get_init_args()` позволяет сохранять среду стандартными агентами вместе с
профилем повреждений. Профиль восстанавливается из JSON-конфигурации;
`reset(options={"damage_profile": profile})` работает и без профиля в конструкторе.
Функцию `damage_event_callback` сериализовать нельзя: сохранение с ней выдаёт
явную ошибку. Сохраняется конфигурация нового эпизода, а не текущее состояние полёта.
...
В `split_stab=True` обе половины ограничиваются до усреднения и вычисления
дифференциальной команды. Дифференциальный момент остаётся упрощённым мгновенным
откликом; отдельной динамики двух приводов эта модель не содержит. Новый
`action_space` меняет масштаб элерона и руля направления: ранее обученные политики
с этими каналами требуют повторной оценки.
