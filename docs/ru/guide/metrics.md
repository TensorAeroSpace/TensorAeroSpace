...
- **Несколько агентов в одном Python-процессе** — каждый writer создаёт независимый W&B-запуск с `reinit="create_new"` и пишет метрики через его объект. Создание или закрытие одного writer оставляет остальные запуски активными. Требуется W&B 0.19.10 или новее; см. [несколько запусков в одном процессе](https://docs.wandb.ai/models/runs/initialize-run). Закрывайте каждый writer после завершения его эксперимента.
