...
!!! note
    Поддерживаемые версии Python: 3.9 — 3.11. Поддержка Python 3.12 планируется.
...
-   :material-apple: **macOS (Intel/M‑series)**
    ...
    На Apple Silicon используйте совместимые версии Python (3.9–3.11) и пакет `tensorflow-macos`.
...
- В проекте используются TensorFlow и PyTorch. Если нужна поддержка GPU, установите фреймворки под вашу платформу:
  ...
  - macOS (Apple Silicon): пакет `tensorflow-macos` устанавливается автоматически (см. зависимости проекта). Убедитесь, что версия Python совместима (3.9–3.11).
