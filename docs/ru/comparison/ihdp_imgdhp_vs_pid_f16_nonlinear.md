...
!!! warning "Историческое сравнение — прежняя реализация"
    Приведённые ниже метрики получены до сверки IHDP/IM-GDHP со статьями. У IM-GDHP изменены критик и цель обучения; требуется переобучение. Эти исторические числа не подтверждают качество и соответствие договору исправленной реализации. Актуальные результаты и ограничения приведены в [уроке SDK](../example/agent/imgdhp/example_imgdhp_nonlinear.md).
    В историческом разделе IM-GDHP не вызывался `learn`: использовался необученный фиксированный актор с внешним интегратором. Эти числа не являются сравнением обученных адаптивных политик IM-GDHP.
