...
\[
\delta_e(t) = u_t \cdot \delta_{e,max}, \quad \delta_{e,max} = 20° \text{ (in rad)},
\]
...
2. The environment clamps \(u_t\) to \([-1, 1]\), maps it to \(\delta_e\) in radians, and advances the internal F-4C model.
...
- The internal state representation follows the order `[u, w, q, theta]` (ft/s, ft/s, rad, rad for the F-4C model), but observations are normalized to `[-1, 1]`.

## References

- `tensoraerospace/envs/f4c.py` -- full environment implementation
- F-4C Phantom II longitudinal dynamics model: `tensoraerospace/aerospacemodel/f4c.py`
