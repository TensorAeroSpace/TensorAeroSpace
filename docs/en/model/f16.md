...
!!! note "Units"
    The Python class `LongitudinalF16` uses `[theta, alpha, q, ele]`: `theta` and `alpha` are in radians, `q` is in rad/s, and `ele` is in degrees. Both `run_step` and the Gymnasium environment accept elevator commands in degrees.

    The retained actuator obeys `d(ele)/dt = 20.2 * (command - ele)`. The aircraft responds to the actuator position. `selected_state_output` selects states in the requested order; `nz` is a separate output recorded in `store_outputs`.

!!! note "Linear angular model"
    `tensoraerospace.aerospacemodel.f16.linear.angular.AngularF16` uses `[phi, theta, psi, alpha, beta, p, q, r, ele, ail, rud]`. Aircraft angles are in radians, body rates in rad/s, and all three actuator positions and commands in degrees. Each retained actuator follows `d(delta)/dt = 20.2 * (command - delta)`.

    `selected_state_output` selects state components, including `ele`, `ail` and `rud`; load factors `nx`, `ny` and `nz` are recorded separately in `store_outputs`. `get_control(..., to_rad=True)` converts command history to radians.
...
!!! tip "Actuator limits"
    ...
    These limits apply to the elevator command in degrees and degrees per second. The actuator position remains part of the state vector.
...
=== "Gymnasium"
    ...
    for _ in range(200):
        action = np.array([0.1], dtype=np.float32)  # deg
...
=== "Model only"
    ...
    for t in range(number_time_steps - 1):
        u = np.array([0.1])  # elevator command (deg)
