...
1. Heffley R. K., Jewell W. F. Aircraft handling qualities data. – NASA, 1972. – No. AD‑A277031. [Link](https://ntrs.nasa.gov/citations/19730003312)
2. Abd Elwahab S. et al. Evaluation of Boeing 747‑E lateral autopilot using flying and handling qualities specifications // ICCCCEE 2017. IEEE. [Link](https://ieeexplore.ieee.org/document/7867653)
...
## Tracking and units in the linear environment

`LinearLongitudinalB747` accepts initial states and references in SI units:
`u,w` in m/s, `q` in rad/s and `theta` in radians. Elevator actions remain in
degrees. Angular observations remain in degrees (and deg/s), so convert them
before comparing directly with a reference in radians.

`output_space` selects the returned observation and its shape. The reward uses
`tracking_states` from the full model state in SI units, independently of
observation selection/order. A one-channel reference tracks the first selected
state; multiple channels follow `tracking_states` order. The last reference
sample is held after its array ends. A 1-D reference is a constant channel vector.
Custom reward callbacks receive the tracked SI state and, when supported, the
clipped command in degrees. The default reward is negative mean squared error.

The time horizon ends with `truncated=True, terminated=False`. This is a time
limit, so learning algorithms can bootstrap the final transition.

**Compatibility:** previous rewards mixed angular observations in degrees with
references in radians and included unrequested observed states. Reward values
and training targets therefore change; re-evaluate old policies before continued
training. Finite state/command validation and copied initial states do not change
the model equations, actuator magnitude limits or actuator rate limits.
