...
def generate_sample_system_response(time, overshoot=0.2, settling_time=3.0, noise_level=0.01):
    """
    ...
    zeta = -np.log(overshoot) / np.sqrt(np.pi**2 + np.log(overshoot)**2)  # Коэффициент затухания
    ...
    for i, t in enumerate(time):
        if t >= 1.0:
            ...
            if zeta < 1.0:  # Недодемпфированная система
                ...
                response = 1 - np.exp(-zeta * wn * tau) * (np.cos(wd * tau) + 
                          (zeta * wn / wd) * np.sin(wd * tau))
            else:  # Передемпфированная система
                r1 = -wn * (zeta + np.sqrt(zeta**2 - 1))
                r2 = -wn * (zeta - np.sqrt(zeta**2 - 1))
                response = 1 + (r2 * np.exp(r1 * tau) - r1 * np.exp(r2 * tau)) / (r2 - r1)
...
def main():
    ...
    metrics = benchmark.plot(
        control_signal, system_signal, 
        signal_val=0.5, dt=dt, tps=time,
    ...
    # Сравниваем системы
    all_metrics = benchmark.compare_systems(
        systems_data, signal_val=0.5, dt=dt
    )
    
    # Выводим сравнительную таблицу в консоль
    print("\n📊 Сравнительная таблица метрик:")
    print("-" * 80)
    print(f"{'Система':<20} {'Перерег.%':<12} {'Время уст.':<12} {'Затухание':<12} {'Стат.ошибка':<12}")
    print("-" * 80)
    
    for system_name, metrics in all_metrics.items():
        print(f"{system_name:<20} {metrics['overshoot']:<12.2f} "
