"""Demo: UFTC on F-16 nonlinear angular with partial loss of the LEFT wing.

Scenario (``WING_STRIKE_LEFT_HALF``): at t=10 s the left wing-tip section is
fully lost AND the left mid-section loses 50 % of its area. The asymmetric
loss of lift on the left side produces a steady rolling moment plus a yaw
coupling, which is harder for the baseline controller than a clean
symmetric failure (e.g. dual-elevator jam).

Run:
    poetry run python \
        example/reinforcement_learning/uftc/uftc_f16_partial_wing_loss_demo.py

Outputs CSV-style trace of FDD severity, roll/sideslip state, and RLS
forgetting factor every 25 steps. Pipe to a plotter or grep ``mode`` to see
the FDD/UFTC mode transitions.
"""
...
def main() -> None:
    ...
    env = NonlinearAngularF16(
        ...
        dt=0.01,
        airspeed=200.0,
        damage_profile=WING_STRIKE_LEFT_HALF,
        split_stab=True,
    )
    obs, _ = env.reset()
    ctl = UFTCController(
    ...
    ref = np.zeros(3)
    print("# t,alpha,beta,p,severity,fault_present,rls_gamma,mode")
    for k in range(n_steps):
        ...
        if terminated or truncated:
            print(
                f"# stopped early at t={k * 0.01:.2f}s "
