...
def _solve_trims():
    trim_result = trim(
        altitude_ft=ALT_REF_FT, V_ft_s=V_REF_FT_S,
    ...
    def residual(z):
        ...
        return np.array([dx[0], dx[1], dx[2], dx[3], dx[4], dx[5], dx[11]],
                        dtype=np.float64)

    z0 = np.array([
        trim_result.alpha_rad, math.radians(-0.44), trim_result.theta_rad,
        delta_e_trim_rad, math.radians(-4.4), math.radians(-2.64),
        throttle_eo_estimate,
    ], dtype=np.float64)
    sol, info, ier, msg = fsolve(residual, z0, full_output=True,
                                 xtol=1e-10, maxfev=2_000)
    ...
    engine_out_action = np.array([de_eo, da_eo, dr_eo, thr_eo],
                                 dtype=np.float64)
...
def _state_feedback_action(x_uftc, engine_loss, healthy_act, eo_act):
    ...
    action[3] += (STATE_FEEDBACK_GAINS["throttle_v"] * v_err
...
def _compose_action(state_feedback, uftc_norm):
    residual = UFTC_RESIDUAL_SCALE * np.clip(
        np.asarray(uftc_norm, dtype=np.float64).reshape(4), -1.0, 1.0,
    )
    return _clip_physical(np.asarray(state_feedback, dtype=np.float64).reshape(4)
                          + residual)
...
def _make_controller(*, lr_actor: float, lr_critic: float,
                     replay_capacity: int, batch_size: int) -> UFTCController:
    cfg = UFTCConfig(
        dt=DT,
        fdd_warmup_steps=0,
        ...
        fdd_cfg=FDDConfig(
            ...
            drift=6.0, h_alarm=15.0,
        ),
        inner_cfg=AAINDIConfig(
            dt=DT, ref_wn=3.0, ref_zeta=0.9,
            u_magnitude_limit=1.0, u_rate_limit=5.0,
            G_init=UFTC_G_INNER,
            ref_error_kp=2.0, ref_error_ki=0.05,
            seed=0,
        ),
...
def _shaped_reward(x_next: np.ndarray, reference: np.ndarray,
                   q_diag: np.ndarray, clip_low: float = -100.0,
                   clip_high: float = 0.0) -> float:
    err = (np.asarray(x_next, dtype=np.float64).reshape(-1)
...
def main() -> None:
    ...
    parser.add_argument("--out", type=str,
                        default="artifacts/dsac/b747_engine_out_v1")
    ...
    print(f"Healthy trim action: de={math.degrees(healthy_act[0]):+.3f} deg, "
          f"throttle={healthy_act[3]:.4f}")
    print(f"Engine-out trim action: de/da/dr={math.degrees(eo_act[0]):+.3f}/"
    ...
    ctl = _make_controller(
        ...
        replay_capacity=args.replay_capacity, batch_size=args.batch_size,
    )
    print(f"DSAC outer: n_state={ctl.l4.cfg.n_state}, "
    ...
    print(f"target_entropy={ctl.l4.target_entropy:.3f}, "
    ...
    for ep in range(int(args.episodes)):
        ...
        for k in range(N_EP - 2):
            ...
            sf_action = _state_feedback_action(x_uftc, engine_loss,
                                               healthy_act, eo_act)
            ...
            x_next = _uftc_state_transform(obs, ref0, engine_loss,
                                           beta_eo_rad)
            ...
            reward = _shaped_reward(x_next, UFTC_REFERENCE, q_diag,
                                    clip_low=-100.0, clip_high=0.0)
            ...
            if total_steps % int(args.log_every) == 0:
                ...
                if not math.isfinite(la) or la > 50.0:
                    print(f"  WARNING: log_alpha.exp() = {la:.3f} — possible saturation")
    ...
    if initial_reward_mean is not None:
        print(f"Initial-episode reward mean: {initial_reward_mean:+.3f} "
