...
def _make_obs(p=0.0, q=0.0, r=0.0, alpha=0.05, beta=0.0, theta=0.0, phi=0.0, V=200.0):
    return {
...
def test_aidi_agent_predict_returns_correct_shape():
    agent = AIDIAgent(
        n_state=3, n_control=3, onboard_ce=_toy_onboard_ce(), config=AIDIConfig(dt=0.01)
    )
...
def test_aidi_agent_full_step_records_metrics():
    ...
    metrics = agent.learn(_make_obs(p=0.01), references=_refs(), time_step=0)
...
def test_aidi_agent_save_load_roundtrip(tmp_path):
    ...
    np.testing.assert_array_equal(loaded._last_G_nominal, agent._last_G_nominal)
