...
def test_improved_elv_initialization(elv_env_default):
    env = elv_env_default
...
def test_improved_elv_reset(elv_env_default):
    env = elv_env_default
    obs, info = env.reset()
...
def test_improved_elv_step_shapes_and_types(elv_env_default):
    env = elv_env_default
    env.reset()
...
def test_improved_elv_action_clamping(elv_env_default):
    env = elv_env_default
    env.reset()
...
def test_improved_elv_initial_action_override_on_first_step():
    initial_state = np.array([0.0, 0.0, 0.0], dtype=np.float32)
    reference_signal = np.zeros((1, 20), dtype=np.float32)
    env = ImprovedELVEnv(
        initial_state=initial_state,
...
def test_improved_elv_get_init_args(elv_env_default):
    env = elv_env_default
    d = env.get_init_args()
...
def test_improved_elv_termination_on_pitch_limit(monkeypatch, elv_env_default):
    env = elv_env_default
    env.reset()

    def _forced_run_step(action):
        # Return a state with theta beyond the allowed pitch envelope
        return np.array([0.0, 0.0, env.max_pitch_rad * 1.1], dtype=np.float32)
