...
from tensoraerospace.envs.b747 import (  # Import the environment from where it is defined
...
def test_step_function(env_setup):
    ...
    np.testing.assert_allclose(
        next_state, np.rad2deg(env.model.xt.reshape(-1)[[3, 2]]), rtol=1e-6
    )
...
def test_reset_function(env_setup):
    ...
    np.testing.assert_allclose(
        state, np.rad2deg(np.asarray(INITIAL_STATE).reshape(-1)[[3, 2]]), rtol=1e-6
    )
