...
from tensoraerospace.envs.ultrastick import LinearLongitudinalUltrastick  # Import the environment from where it is defined
...
@pytest.fixture
def env_setup():
    return LinearLongitudinalUltrastick(initial_state=INITIAL_STATE, 
...
def test_initialization(env_setup):
    ...
    assert len(env.initial_state) == 5, "Initial state shape should match input."
...
#     env = env_setup
#     action = np.array([10,5], dtype=np.float32)  # within the valid range
...
#     env = env_setup
#     env.step(np.array([10,5], dtype=np.float32))  # change state
