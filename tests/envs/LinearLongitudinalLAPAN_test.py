...
from tensoraerospace.envs.lapan import LinearLongitudinalLAPAN  # Import the environment from where it is defined
...
@pytest.fixture
def env_setup():
    return LinearLongitudinalLAPAN(initial_state=INITIAL_STATE, 
...
def test_step_function(env_setup):
    ...
    assert isinstance(reward, (float, np.floating, np.ndarray)), "Reward should be a float or array."
