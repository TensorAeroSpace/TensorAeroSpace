...
def test_aero_lift_slope_matches_published_CLa():
    ...
    p = default_parameters()
    common = dict(beta=0.0, V=18.0, p=0.0, q=0.0, r=0.0,
                  altitude_m=178.0, de=0.0, da=0.0, CT=0.0)
...
def test_aero_elevator_creates_pitch_down_at_positive_de():
    p = default_parameters()
    common = dict(alpha=math.radians(4.0), beta=0.0, V=18.0,
                  p=0.0, q=0.0, r=0.0, altitude_m=178.0,
                  da=0.0, CT=0.0)
...
def test_aero_sideslip_gives_restoring_yaw():
    ...
    p = default_parameters()
    common = dict(alpha=math.radians(4.0), V=18.0,
                  p=0.0, q=0.0, r=0.0, altitude_m=178.0,
                  de=0.0, da=0.0, CT=0.0)
...
def test_env_make_via_gym_registry_works():
    env = gym.make("NonlinearSkywalkerX8-v0",
                   trim_at=(178.0, 18.0), number_time_steps=10).unwrapped
    obs, _ = env.reset()
    assert obs.shape == (12,)
...
def test_env_holds_trim_briefly():
    ...
    r = trim(altitude_m=178.0, V_m_s=18.0)
    env = NonlinearSkywalkerX8Env(initial_state=r.to_state(),
                                    number_time_steps=200, dt=0.01)
    obs, _ = env.reset()
