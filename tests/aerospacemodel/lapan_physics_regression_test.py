"""Physical actuator, observation and history contracts for LAPAN."""
...
from tensoraerospace.envs.lapan import ImprovedLAPANEnv, LinearLongitudinalLAPAN


def test_first_step_obeys_elevator_rate_limit():
    model = LAPAN(np.zeros(4), 5, dt=0.01)
...
def test_initial_actuator_position_is_preserved_after_reset():
    model = LAPAN(np.zeros(4), 5, dt=0.01, initial_control=np.deg2rad(5))
    for _ in range(2):
        model.run_step(np.array([0]))
        assert np.rad2deg(model.store_input[0, 0]) == pytest.approx(2.0)
        model.initialise_system(np.zeros(4), 5)


def test_initial_state_is_not_aliased():
    initial = np.array([0.1, 0.2, 0, 0.01])
    model = LAPAN(initial, 5)
...
def test_linear_reward_tracks_pitch_from_full_model_state():
    env = LinearLongitudinalLAPAN([0.1, 0.2, 0.01, 0.02], np.zeros((1, 4)), 4)
    obs, reward, _, _, _ = env.step([0])
    assert reward == pytest.approx(-abs(env.model.xt[3, 0]))
    np.testing.assert_allclose(obs, env.model.xt.ravel()[[3, 2]])


def test_linear_output_selection_and_multichannel_reward():
    env = LinearLongitudinalLAPAN(
        [0.1, 0.2, 0.01, 0.02],
        np.zeros((2, 4)),
        4,
        state_space=["u", "w"],
...
def test_linear_dt_reference_and_time_limit():
    env = LinearLongitudinalLAPAN(np.zeros(4), lambda t: t, 3, dt=0.02)
...
def test_linear_reset_owns_initial_state_and_reference():
    ...
    env = LinearLongitudinalLAPAN(initial, reference, 3)
    initial[:] = 99
    reference[:] = 99
    np.testing.assert_array_equal(env.reset()[0], np.zeros(2))
...
def normalized(**kwargs):
    ...
    return ImprovedLAPANEnv(**defaults)
...
def test_normalized_observation_and_reward_use_applied_elevator():
    ...
    assert info["elevator_deg"] == pytest.approx(3.0)
    cost = (
...
def test_normalized_initial_elevator_sets_physical_actuator_state():
    ...
    for _ in range(2):
        assert env.reset()[0][3] == pytest.approx(0.2)
        env.step([0])
        assert np.rad2deg(env.model.store_input[0, 0]) == pytest.approx(2.0)


def test_normalized_pitch_envelope_and_last_reward():
    env = normalized(initial_state=[0, 0, 0, np.deg2rad(21)])
...
def test_invalid_environment_action_does_not_advance(action, kind):
    env = (
        normalized()
        if kind == "normalized"
        else LinearLongitudinalLAPAN(np.zeros(4), np.zeros((1, 4)), 4)
    )
    with pytest.raises(ValueError):
        env.step(action)
...
def test_invalid_initial_actuator_position_is_rejected(initial_control):
    with pytest.raises(ValueError):
        LAPAN(np.zeros(4), 4, initial_control=initial_control)
...
def test_normalized_clips_initial_actuator_consistently():
    ...
    env.step([0])
    assert np.rad2deg(env.model.store_input[0, 0]) == pytest.approx(25)
...
def test_invalid_reference_is_rejected(kind, reference):
    cls = LinearLongitudinalLAPAN if kind == "linear" else ImprovedLAPANEnv
    with pytest.raises(ValueError):
        cls(np.zeros(4), reference, 4)


def test_legacy_action_is_degrees_before_rate_limiting():
    env = LinearLongitudinalLAPAN(np.zeros(4), np.zeros((1, 4)), 4)
    env.step([1.0])
    assert np.rad2deg(env.model.store_input[0, 0]) == pytest.approx(1.0)
