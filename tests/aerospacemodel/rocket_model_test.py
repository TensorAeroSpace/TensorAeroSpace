...
def test_rocket_multiple_steps():
    """Cover time_step != 0 branch."""
    model = MissileModel(x0=np.zeros(4), number_time_steps=5, dt=0.01)
...
def test_rocket_get_state_conversions():
    """Cover to_deg/to_rad branches and aliases (wz->q, wx->p, wy->r)."""
    model = MissileModel(x0=np.zeros(4), number_time_steps=5, dt=0.01)
...
def test_rocket_get_control_conversions():
...
def test_rocket_get_output_basic():
    """Cover get_output without conversions (to_deg/to_rad have bugs in prod code)."""
    model = MissileModel(x0=np.zeros(4), number_time_steps=5, dt=0.01)
    model.run_step(np.array([1.0]))
    model.run_step(np.array([1.0]))
