...
from tensoraerospace.agent.a2c.model import A2C, Actor, Critic, Mish  # Assume importing A2C and necessary exceptions
...
def mock_environment():
    ...
    model = A2C(env=env, actor=actor, critic=critic, gamma=0.9, entropy_beta=0.01, actor_lr=4e-4, critic_lr=4e-3)
    return model

def test_get_param_env(mock_environment):
    param_env = mock_environment.get_param_env()
    
    assert param_env["env"]["name"] == "gymnasium.envs.classic_control.pendulum.PendulumEnv"
    assert param_env["policy"]["name"] == f"{mock_environment.__module__}.{mock_environment.__class__.__name__}"
...
def test_save(mock_environment):
    ...
    with open(model_dir / "config.json") as f:
        config = json.load(f)
    assert config["policy"]["name"] == f"{mock_environment.__module__}.{mock_environment.__class__.__name__}"
